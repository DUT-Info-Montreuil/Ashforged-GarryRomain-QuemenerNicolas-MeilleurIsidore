...
public class Character extends Entity {
    ...
    public void attack() {
        if (getHoldingItem() instanceof ItemStock.Weapon) {
            for (Entity entity : Environment.getInstance().getNonePlayer()) {
                if (entity instanceof Ennemis) {
                    ...
                    if (dx < 2 && dy < 2) { // à portée d'attaque
                        ...
                        int damage;
                        if (this.power > 1) {
                            damage = (int) (this.power * 0.5 + ((double) ((ItemStock.Weapon) getHoldingItem()).getDamage() / 2));
                        } else {
                            damage = ((ItemStock.Weapon) getHoldingItem()).getDamage() / 2;
                        }
